    ...
    <header className="relative w-full px-2 pb-3 pt-3 md:px-4">
      <div className="mb-4 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <ActionButton variant="ghost" onClick={() => setIsInfoOpen((prev) => !prev)}>
      ...
      <div className="mt-5 flex flex-wrap items-center gap-3 text-[0.9rem] text-[#49565c] sm:text-base">
        ...
        <span className="inline-flex items-center gap-2">
          <span aria-hidden="true" className="text-base leading-none text-[#7a6a50]">
            ↻
          </span>
          Auto-refresh 30s
        </span>
        ...
        <div className="absolute right-2 top-23 z-20 w-[min(92vw,440px)] rounded-[18px] border border-[#d8d0c5] bg-[#f2efe9] p-5 shadow-[0_18px_40px_rgba(13,35,34,0.12)] md:right-4">
          ...
          <div className="space-y-4">
            {dataSources.map((item) => (
              <div key={item.name} className="flex items-start justify-between gap-3 border-b border-[#d8d1c8] pb-4 last:border-b-0 last:pb-0">
                <div className="flex items-start gap-3">
                  <div
                    className={`mt-1 flex h-10 w-10 items-center justify-center rounded-xl text-lg font-bold text-white ${
                      item.badge === "purple"
                        ? "bg-[#7a5ad8]"
                        : item.badge === "green"
                          ? "bg-[#39a35d]"
                  ...
                  <div className="flex-1">
                    <div className="text-[1.05rem] font-semibold text-[#1d2d2d]">
                      {item.name}
                    </div>
                    {item.subtitle ? (
